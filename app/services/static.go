...
// ListInnovations returns paginated innovations.
func (s *StaticService) ListInnovations(ctx context.Context, category string, page, perPage int) (*PaginationResult, error) {
	page, perPage, offset := NormalizePagination(page, perPage)
	...
	rows, err := s.querier.ListInnovationsPaginated(ctx, queries.ListInnovationsPaginatedParams{
		Column1: catArg,
		Limit:   int64(perPage),
		Offset:  int64(offset),
	})
	...
	items := make([]InnovationItem, 0, len(rows))
	for _, r := range rows {
		summary := ""
		if r.Summary.Valid {
		...
		items = append(items, InnovationItem{
			ID:        r.ID,
			Title:     r.Title,
			Year:      r.Year,
			Category:  r.Category,
	...
	total, err := s.querier.CountInnovationsFiltered(ctx, catArg)
	if err != nil {
		return nil, err
	}
	return BuildPagination(items, page, perPage, total), nil
...
// GetInnovation returns a single innovation.
func (s *StaticService) GetInnovation(ctx context.Context, id int64) (*InnovationItem, error) {
	r, err := s.querier.GetInnovationByID(ctx, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
